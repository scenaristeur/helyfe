<template>
    <b-container v-if="showBrainLoader == true">
        ...
        <b-list-group v-if="brains != undefined" class="scroll">
            <b-list-group-item v-for="b in items" :key="b.id" @click="loadBrain(b)"
                class="item list-group-item d-flex justify-content-between p-1" button>
                <p class="p-0 m-0 flex-grow-1">
                    <b>{{ b.name }}</b>
                </p>
                <small class="mr-2"><i>created: {{ new Date(b.created).toLocaleString() }} <br> updated: {{ new
                    Date(b.updated).toLocaleString() }}</i></small>

                <!-- <a :href="b.id" target="_blank" button  size="sm mr-2" variant="outline-success"><b-icon-link></b-icon-link></a> -->
                <b-button @click.stop="open(b.id)" size="sm mr-2"
                    variant="outline-success"><b-icon-eye></b-icon-eye></b-button>
                <b-button @click.stop="share(b.id)" size="sm mr-2"
                    variant="outline-success"><b-icon-share></b-icon-share></b-button>
