...
-->
    <b-input-group>
        <b-input id="input" ref="input" type="search" autocomplete="off" v-model="main_input"
            v-on:keydown.9.prevent="onEnter" v-on:keydown.enter="onEnter" v-on:paste="onPaste" v-on:input="onInput"
            title="type three words followed by a comma"
            placeholder="three words followed by a comma, or /h +Enter for help">
        </b-input>
        <template #append>
            <b-form-select v-if="orderedNodes.length > 0" v-model="selected" value-field="id" text-field="name"
