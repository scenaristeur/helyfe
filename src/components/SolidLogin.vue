<template>
    <span class="login">
        ...
        {{issuer.text}}
                ...
                <b-form-select autofocus size="sm" v-model="issuer" :options="issuers" @change="login">
                    <template #first>
                ...
                <div v-if="issuer == 'other'">
                    <b-form-group id="fieldset-1" description="ex: `https://solidcommunity.net`"
                        label="url of your pod provider" label-for="input-1" valid-feedback="Thxs!"
                        :invalid-feedback="invalidFeedback" :state="state">

                        <b-form-input id="input-1" v-model="other" :state="state" @change="onChange"
                            trim></b-form-input>
                    </b-form-group>
...
export default {
    ...
    computed: {
        session: {
            get() {
                // Vérification sécurisée de l'existence de l'état solid
                if (this.$store && this.$store.state && this.$store.state.solid) {
